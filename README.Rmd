...
"blue" shown in `r colorize("blue")`. This is similar to the use of **direct labels** in graphs, which are perceptually bound 
...
* `colorize_bg(text, color)`: Print the `text` with a background in `color`
* `colorbox(text, color, ...)`: Print `text` with bacground in color, but where the text is given a contrasting color designed to be most legible.
